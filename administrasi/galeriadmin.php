...
if ($p == "simpan_album") {
	...
	if ($QTambahKategori) {
		echo "<script>alert('Berhasil Ditambahkan'); window.open('?page=galeriadmin', '_self');</script>";
	} else {
		echo "<script>alert('Gagal Ditambahkan'); window.open('?page=galeriadmin', '_self');</script>";
	}
}
...
if ($mod == "del_kat") {
	...
	if ($QDelKategori && $QDelGaleriKat) {
		echo "<script>alert('Berhasil Dihapuskan'); window.open('?page=galeriadmin', '_self');</script>";
	} else {
		echo "<script>alert('Gagal Dihapus'); window.open('?page=galeriadmin', '_self');</script>";
	}
} else if ($mod == "upload") {
	$id_kat		= $_POST['id_kat'];
	$ket		= $_POST['ket'];
	...
	if ($fileType == "image/gif" || $fileType == "image/pjpeg" || $fileType == "image/jpeg") {
		...
		echo "<script>alert('Berhasil Ditambahkan'); window.open('?page=galeri_form?id_kat=".$_POST['kategori']."', '_self');</script>";
			
	} else {
		echo "<script>alert('Gagal Ditambahkan'); window.open('?page=galeri_form?id_kat=".$_POST['kategori']."', '_self');</script>";
	}
}
